        ...
        while True:
            ...
            try:
                ...
                if not os.path.exists(self.priv_path): # or not os.path.exists(self.pub_path):
                    ...
                    with open(self.priv_path, 'wb') as pk:
                        pk.write(self.priv_key.exportKey('PEM'))
                else:
                ...
                if not os.path.exists(self.pub_path):
                    self._view.on_keys_generation()
                    logging.info("generating %s ..." % self.pub_path)
                    self.pub_key = self.priv_key.publickey()
                    with open(self.pub_path, 'wb') as pubk:
                        pubk.write(self.pub_key.exportKey('PEM'))
                else:
                ...
                logging.error("error loading keys, maybe corrupted, deleting and regenerating ...")
                try:
